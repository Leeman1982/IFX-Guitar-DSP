 ...
 *
 * NOTE: opcodes verified with the pico-SDK encoder (pio_encode_*).
 *   Two bugs were fixed vs. the earlier hand-assembly:
...
static inline void i2s_out_program_init(PIO pio, uint sm, uint offset,
     ...
     * target = 48000 * 65 * 2 makes LRCK exactly 48 kHz and BCK = 3.072 MHz. */
 ...
 *
 * In-base pin 0 = DOUT (serial data from ADC)        = GP19
 * In-base pin 1 = LRCK (bridged from output GP18)    = GP20
 * In-base pin 2 = BCK  (bridged from output GP17)    = GP21
 *
 * No side-set, runs at system clock to catch BCK edges.
 *
 * NOTE: opcodes verified with the pico-SDK encoder. The two jmp x--
 *   instructions previously decoded as jmp PIN (cond 0b110) and one also
 *   targeted the wrong address (set x,31 instead of the loop body),
 *   producing an infinite loop and no captured samples. Both fixed.
 *
 * wait_left:
