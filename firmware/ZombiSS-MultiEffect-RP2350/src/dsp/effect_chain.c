...
static void init_param_descs(EffectChain *ec) {
    ...
    ec->params[FX_EQ][EQ_CENTER_FREQ] = (ParamDesc){"Freq",      80.0f, 12000.0f, 20.0f, 1000.0f, "Hz"};
    ec->params[FX_EQ][EQ_BANDWIDTH]   = (ParamDesc){"BW",        50.0f, 5000.0f,  10.0f, 500.0f,  "Hz"};
    ec->params[FX_EQ][EQ_BOOST_CUT]   = (ParamDesc){"Gain",      0.1f,  4.0f,     0.05f, 1.0f,    "x"};
...
void EffectChain_Init(EffectChain *ec) {
    ...
    IFX_PeakingFilter_Init(&ec->eq, SAMPLE_RATE_HZ);
    IFX_PeakingFilter_SetParameters(&ec->eq,
        ec->params[FX_EQ][EQ_CENTER_FREQ].value,
        ec->params[FX_EQ][EQ_BANDWIDTH].value,
        ec->params[FX_EQ][EQ_BOOST_CUT].value);
...
float EffectChain_Process(EffectChain *ec, float inp) {
    ...
    /* 1. Noise Gate */
    if (ec->active[FX_NOISEGATE]) {
        sig = IFX_NoiseGate_Update(&ec->noiseGate, sig);
    }

    /* 2. Overdrive */
    if (ec->active[FX_OVERDRIVE]) {
        sig = IFX_Overdrive_Update(&ec->overdrive, sig);
    }

    /* 3. Peaking EQ */
    if (ec->active[FX_EQ]) {
        sig = IFX_PeakingFilter_Update(&ec->eq, sig);
    }

    /* 4. Chorus */
    if (ec->active[FX_CHORUS]) {
        sig = IFX_Chorus_Update(&ec->chorus, sig);
    }

    /* 5. Delay */
    if (ec->active[FX_DELAY]) {
        sig = IFX_Delay_Update(&ec->delay, sig);
    }
...
void EffectChain_SetParam(EffectChain *ec, uint8_t fxIndex, uint8_t paramIndex, float value) {
    ...
    case FX_NOISEGATE:
        switch (paramIndex) {
        case NG_THRESHOLD:
            IFX_NoiseGate_SetThreshold(&ec->noiseGate, value);
            break;
        case NG_ATTACK:
        case NG_RELEASE:
            ...
            break;
        case NG_HOLD:
            IFX_NoiseGate_SetHoldTime(&ec->noiseGate, value);
            break;
        }
        break;
    ...
    case FX_OVERDRIVE:
        switch (paramIndex) {
        case OD_GAIN:
            IFX_Overdrive_SetGain(&ec->overdrive, value);
            break;
        case OD_BOOST:
            IFX_Overdrive_SetBoost(&ec->overdrive, value);
            break;
        case OD_HPF_CUTOFF:
            IFX_Overdrive_SetHPF(&ec->overdrive, value);
            break;
        case OD_LPF_CUTOFF:
        case OD_LPF_DAMP:
            ...
            break;
        case OD_Q_CLIP:
            IFX_Overdrive_SetQ(&ec->overdrive, value);
            break;
        }
        break;
    ...
    case FX_CHORUS:
        ...
        case CH_GAIN_B:
            ...
            break;
        case CH_MIX:
            IFX_Chorus_SetMix(&ec->chorus, value);
            break;
        default:
            /* Delay time changes require re-init of chorus - handled at init only */
            break;
        }
        break;
    ...
    case FX_DELAY:
        switch (paramIndex) {
        case DL_TIME:
            IFX_Delay_SetLength(&ec->delay, value, SAMPLE_RATE_HZ);
            break;
        case DL_MIX:
            IFX_Delay_SetMix(&ec->delay, value);
            break;
        case DL_FEEDBACK:
            IFX_Delay_SetFeedback(&ec->delay, value);
            break;
        }
        break;
    }
