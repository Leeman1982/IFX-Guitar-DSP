...
static void init_param_descs(EffectChain *ec) {
    ...
    ec->params[FX_OVERDRIVE][OD_LPF_CUTOFF] = (ParamDesc){"LPF",      500.0f,16000.0f,100.0f,5000.0f,"Hz"};
    ...
    ec->params[FX_CHORUS][CH_DELAY_A] = (ParamDesc){"Delay A",   1.0f,  30.0f,  0.5f,  10.0f, "ms"};
    ec->params[FX_CHORUS][CH_DELAY_B] = (ParamDesc){"Delay B",   1.0f,  30.0f,  0.5f,  15.0f, "ms"};
    ec->params[FX_CHORUS][CH_DEPTH_A] = (ParamDesc){"Depth A",   0.0f,  15.0f,  0.5f,  8.0f,  ""};
    ec->params[FX_CHORUS][CH_DEPTH_B] = (ParamDesc){"Depth B",   0.0f,  15.0f,  0.5f,  8.0f,  ""};
    ...
    /* RP2040: max delay ~375ms at 32kHz with 12000-sample buffer */
    ec->params[FX_DELAY][DL_TIME]     = (ParamDesc){"Time",      10.0f, 375.0f, 5.0f,  250.0f, "ms"};
...
float EffectChain_Process(EffectChain *ec, float inp) {
    ...
    if (ec->active[FX_EQ])       sig = IFX_PeakingFilter_Update(&ec->eq, sig);
...
void EffectChain_SetParam(EffectChain *ec, uint8_t fxIndex, uint8_t paramIndex, float value) {
    ...
    case FX_NOISEGATE:
        ...
        case NG_ATTACK: case NG_RELEASE:
            IFX_NoiseGate_SetAttackRelease(&ec->noiseGate,
                ec->params[FX_NOISEGATE][NG_ATTACK].value,
                ec->params[FX_NOISEGATE][NG_RELEASE].value, SAMPLE_RATE_HZ); break;
    ...
    case FX_EQ:
        IFX_PeakingFilter_SetParameters(&ec->eq,
            ec->params[FX_EQ][EQ_CENTER_FREQ].value,
            ec->params[FX_EQ][EQ_BANDWIDTH].value,
            ec->params[FX_EQ][EQ_BOOST_CUT].value); break;
    case FX_CHORUS:
        switch (paramIndex) {
