...
void SH1106::init(uint8_t sda_pin, uint8_t scl_pin, uint8_t addr) {
    ...
    sendCmd(0xDA); sendCmd(0x12);  /* COM pins */
    sendCmd(0x81); sendCmd(0xCF);  /* Contrast */
    sendCmd(0xD9); sendCmd(0xF1);  /* Pre-charge */
    sendCmd(0xDB); sendCmd(0x40);  /* VCOMH */
...
void SH1106::flush() {
    for (uint8_t page = 0; page < SH1106_PAGES; page++) {
        ...
        /* Wire library buffer is typically 128 bytes on arduino-pico,
           send in two chunks to be safe */
        Wire.beginTransmission(_addr);
        Wire.write(0x40);  /* Co=0, D/C#=1 (data) */
        Wire.write(&_framebuf[page * SH1106_WIDTH], SH1106_WIDTH);
        Wire.endTransmission();
    }
}
